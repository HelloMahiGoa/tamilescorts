...
export default function ProfileDetailClient({
  ...
  const availableVideos = profile.videos?.filter(v => !failedVideos.has(v)) || [];
  const maskInstagram = (handle: string) => {
    if (handle.length <= 6) return handle.slice(0, 2) + "***";
    const showStart = Math.min(3, Math.floor(handle.length / 3));
    const showEnd = Math.min(3, Math.floor(handle.length / 4));
    return handle.slice(0, showStart) + "***" + handle.slice(-showEnd);
  };
          ...
          <div className="grid gap-8 lg:grid-cols-12 lg:gap-12">
            ...
            <div className="lg:col-span-5">
              <div className="lg:sticky lg:top-8 space-y-4">
                ...
                {/* Videos - some profiles */}
                {availableVideos.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-sm font-semibold text-white/90 uppercase tracking-wide">Videos</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availableVideos.map((src, idx) => (
                        <div
                          key={src}
                          className="group relative aspect-video overflow-hidden rounded-xl border border-white/10 bg-black/50 transition-all hover:border-[var(--accent-gold)]/50"
                        >
                          <video
                            src={src}
                            controls
                            ...
                            onError={() => setFailedVideos(prev => new Set(prev).add(src))}
                          />
              ...
              <div className="rounded-2xl border border-[var(--accent-gold)]/30 bg-gradient-to-br from-[var(--accent-gold)]/10 to-[var(--accent-gold)]/5 p-8 backdrop-blur-sm">
                <div className="text-center">
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-4">
                    ...
                    {profile.instagram && (
                      <div className="inline-flex flex-col items-center gap-2 rounded-xl border border-white/20 bg-white/5 px-6 py-4">
                        <div className="flex items-center gap-2 text-white/80">
                          ...
                          <span className="font-mono text-base">@{maskInstagram(profile.instagram)}</span>
                        </div>
                        <p className="text-xs text-white/60">Contact us via Telegram to reveal full ID</p>
                        <a
                          href={telegramUrl}
                          target="_blank"
                          rel="noopener noreferrer"
